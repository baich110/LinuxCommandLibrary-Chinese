...
actual fun rememberOpenAppAction(): (String) -> Unit {
    ...
    return remember(context) {
        { action ->
            try {
                when (action) {
                    "settings" -> context.startActivity(Intent(Settings.ACTION_SETTINGS).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) })
                    "terminal" -> {
                        val intent = context.packageManager.getLaunchIntentForPackage("com.android.virtualization.terminal")
